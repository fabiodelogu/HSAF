...
import Lib_Analysis_Computation_Generic as Lib_Analysis_Computation
import Lib_Analysis_Regression as Lib_Analysis_Regression
import Lib_Op_Exec_Apps as Lib_OpExec
...
def interpVarPointZReg(oDataGeo, oDataVar, oGeoXVar, oGeoYVar, oGeoZVar, 
    ...
    if oGeoXVar != None and oGeoYVar != None:
        ...
        if a1iIndexSel is not None:
            ...
            a2dDataVarInterp = interpVarPointIDW(oDataGeo, a1dDataVarRes,
            ...
            a2dDataVarInterp = a2dGeoDataZPoly + a2dDataVarInterp
        ...
        Exc.getExc(' -----> WARNING: all data are none! Data are not available!', 2, 1)
        a2dDataVarInterp = np.zeros((iRowsRef, iColsRef))
        a2dDataVarInterp[:] = np.nan

    # Return results
    return a2dDataVarInterp
...
def interpVarPointIDW(oDataGeo, oDataVar, oGeoXVar, oGeoYVar, oGeoZVar, 
    ...
    if oGeoXVar != None and oGeoYVar != None:
        ...
        if a1iIndexSel is not None:
            ...
            [sStdOut, sStdErr, sStdCode] = Lib_OpExec.runExec(sCommandLine_EXEC=sLineCommand, sRunName_EXEC='interpVarPointIDW')

            # Read tiff file
            oFileDriver_TIFF = Drv_Data_IO(join(sPathTemp, sFileNameTiff), 'r').oFileWorkspace
            oFileData = oFileDriver_TIFF.oFileLibrary.openFile(join(oFileDriver_TIFF.sFilePath, oFileDriver_TIFF.sFileName),
                                                                    oFileDriver_TIFF.sFileMode)
            a2dDataVarInterp = oFileDriver_TIFF.oFileLibrary.read2DVar(oFileData, iRowsRef, iColsRef)
            oFileDriver_TIFF.oFileLibrary.closeFile(oFileData)
...
def interpVarPointNN(oGeoX_OUT, oGeoY_OUT, oGeoNan_OUT,
                     dRadiusX_OUT, dRadiusY_OUT,
                     oData_IN, oGeoX_IN, oGeoY_IN,
                     oGeoZ_IN=None,
                     iEPSGCode=None, sPathTemp=None):
    ...
    iColsRef = oGeoX_OUT.shape[1]
    iRowsRef = oGeoY_OUT.shape[0]
    a2bGeoDataNan = oGeoNan_OUT

    # Check temporary folder
    if not sPathTemp:
        sPathTemp = os.path.dirname(os.path.abspath(__file__))
    else:
        pass
    ...
    if oGeoXVar != None and oGeoXVar != None:
        ...
        if a1iIndexSel is not None:
            ...
            # Grid option(s)
            sOption = ('-a nearest:radius1=' + str(dRadiusX_OUT) + ':radius2=' +
                        str(dRadiusY_OUT) + ':angle=0.0:nodata=' + str(dNoData))
            ...
            oData = {}
            oData['data'] = a2dDataSel
            sio.savemat('oSSM_PointData.mat', oData)
            ...
            [sStdOut, sStdErr, sStdCode] = Lib_OpExec.runExec(sCommandLine_EXEC=sLineCommand, sRunName_EXEC='interpVarPointNN')
...
def interpVarPointMLR(oDataGeo, oDataVar, oGeoXVar, oGeoYVar, oGeoZVar, 
    ...
    a2dVarInterp = interpVarPointIDW(oDataGeo, a1dVarRes,
                                        a1dVarX, a1dVarY, a1dVarZ,
                                        dRadiusX, dRadiusY, dRadiusInfluence,
                                        iEPSGCode, sPathTemp, oDataAnalyzed)
    
    # Final variable estimation (Snow Hweight in [cm])
    a2dVarFinal = a2dVarMap + a2dVarInterp
    ...
    a2dVarW = Lib_Analysis_Computation.computeVarKernel(oDataGeo,
...
def interpVarGridNN(oGeoX_OUT, oGeoY_OUT, oGeoNan_OUT,
                    oData_IN, oGeoX_IN, oGeoY_IN,
                    dNoData_OUT=np.nan, a2iIndex_OUT=None, bGeoCut_OUT=False):
    ...
    if a2iIndex_OUT:
        ...
        a2dDataVarRegrid = griddata((a2dGeoXVar_SUB.ravel(), a2dGeoYVar_SUB.ravel()), a2dDataVar_SUB.ravel(),
                                    (a2dGeoXRef, a2dGeoYRef), method='nearest', fill_value=dNoData_OUT)
