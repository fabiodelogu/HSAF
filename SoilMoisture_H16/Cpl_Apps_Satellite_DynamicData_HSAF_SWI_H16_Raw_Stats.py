...
Name:          Cpl_Apps_Satellite_DynamicData_SWI_H16_Raw_Stats
...
# -------------------------------------------------------------------------------------
# Class
class Cpl_Apps_Satellite_DynamicData_HSAF_SWI_H16_Raw_Stats:
    ...
    def checkDynamicData(self, sTime):
        ...
        for sFileType in oVarsWorkspace_CHK:
            ...
            if sFileType == 'NetCDF':
                ...
                # Check file
                sFileCache_CHK = os.path.join(sPathCache_CHK, 'Satellite_HSAF-SWI-H16-RAW-STATS_FILENC_' + sTime_CHK + '.history')
    ...
    def getDynamicData(self, sTime):
        ...
        #oData_GET = open('oDataDyn_GET_H16-STATS.pkl', 'wb')
        #pickle.dump(a1oFileDataDyn_GET, oData_GET)
        #oData_GET.close()
        #oGeo_GET = open('oTimeDyn_GET_H16-STATS.pkl', 'wb')
        #pickle.dump(a1oFileDataGeo_GET, oGeo_GET)
        #oGeo_GET.close()
        #import scipy.io as sio
        #os.chdir(self.sPathClass)
        #oData = {'SWI': a1oFileDataDyn_GET['200907010000']}
        #sio.savemat('oDataDyn_GET_H16-STATS.mat', oData)
        # DEBUG END #####
    ...
    def parserDynamicData(self, sTime):
        ...
        #import pickle
        #os.chdir(self.sPathClass)
        #oData_GET = open('oDataDyn_GET_H16-STATS.pkl', 'rb')
        #a1oFileDataDyn_GET = pickle.load(oData_GET)
        #oData_GET.close()
        #oGeo_GET = open('oTimeDyn_GET_H16-STATS.pkl', 'rb')
        ...
        #import pickle
        #os.chdir(self.sPathClass)
        #oData_PAR = open('oDataDyn_PAR_H16-STATS.pkl', 'wb')
        #pickle.dump(a1oFileDataDyn_PAR, oData_PAR)
        #oData_PAR.close()
        #oTime_PAR = open('oTimeDyn_PAR_H16-STATS.pkl', 'wb')
        #pickle.dump(a1oFileTimeDyn_PAR, oTime_PAR)
        #oTime_PAR.close()
        #import scipy.io as sio
        #os.chdir(self.sPathClass)
        #oData = {'SWI': a1oFileDataDyn_PAR}
        #sio.savemat('oDataDyn_PAR_H16-STATS.mat', oData)
        # DEBUG END #####
    ...
    def computeDynamicData(self, sTime):
        ...
        #import pickle
        #os.chdir(self.sPathClass)
        #oFile_PAR = open('oDataDyn_PAR_H16-STATS.pkl', 'rb')
        #a1oFileDataDyn_PAR = pickle.load(oFile_PAR)
        #oFile_PAR.close()
        #os.chdir(self.sPathClass)
        #oTime_PAR = open('oTimeDyn_PAR_H16-STATS.pkl', 'rb')
        ...
        #import pickle
        #os.chdir(self.sPathClass)
        #oData_CMP = open('oDataDyn_CMP_H16-STATS.pkl', 'wb')
    ...
    def saveDynamicData(self, sTime):
        ...
        #import pickle
        #os.chdir(self.sPathClass)
        #oFile_PAR = open('oDataDyn_CMP_H16-STATS.pkl', 'rb')
        ...
        if a1oFileDataDyn_CMP:
                ...
                if sFileType == 'NetCDF':
                    ...
                    for sVarName_OUT in oVarInfo_OUT:
                        ...
                        try:
                            ...
                            if not bFileExistDyn_OUT:
                                ...
                                bVarExistNC_OUT = oFileDrv_OUT.oFileLibrary.checkVarName(oFileData_OUT,
                                                                                         sVarName_OUT)
                                # -------------------------------------------------------------------------------------
                                ...
                                try:
                                    ...
                                    oFileDRV_WriteMethod = getattr(oFileDrv_OUT.oFileLibrary,
                                                                   oVarsInfo_OUT[sVarGeoX]['VarOp']['Op_Save'][
                                                                       'Func'])
                                ...
                                try:
                                    ...
                                    # Get latitude
                                    oFileDRV_WriteMethod = getattr(oFileDrv_OUT.oFileLibrary,
                                                                   oVarsInfo_OUT[sVarGeoY]['VarOp']['Op_Save'][
                                                                       'Func'])
                                ...
                                oLogStream.info(
                                    ' ------> Saving file output NetCDF: ' + sFileNameDyn_OUT + ' ... OK')
                                # -------------------------------------------------------------------------------------
                                ...
                                oZipDrv_OUT = Drv_File_Zip((sFileNameDyn_OUT + '.gz'), 'u', None,
                                                           '.gz').oFileWorkspace
                                [oFileZip_IN, oFileZip_OUT] = oZipDrv_OUT.oFileLibrary.openZip(
                                    oZipDrv_OUT.sFileName_IN,
                                    oZipDrv_OUT.sFileName_OUT,
                                ...
                                if bVarExistNC_OUT is False:
                                    ...
                                    try:
                                        ...
                                        # Get data dynamic
                                        oFileDRV_WriteMethod = getattr(oFileDrv_OUT.oFileLibrary,
                                                                       oVarsInfo_OUT[sVarName_OUT]['VarOp'][
                                                                           'Op_Save'][
                                                                           'Func'])
                                        oFileDRV_WriteMethod(oFileData_OUT,
                                                             sVarName_OUT, a2VarData_OUT,
                                        ...
                                        # Exit code
                                        a1bFileCheck_HIS.append(False)
                                        Exc.getExc(' -----> WARNING: variable not found in outcome workspace!', 2,
                                                   1)
                                        oLogStream.info(
                                            ' -------> Saving variable: ' + sVarName_OUT + ' ... FAILED --- VARIABLE NOT FOUND IN OUTCOME WORKSPACE ')
                                        # -------------------------------------------------------------------------------------
                            ...
                            a1bFileCheck_HIS.append(False)
                            # Info
                            Exc.getExc(' ------> WARNING: errors occurred in saving file! Check your output data!',
                                       2, 1)
                            oLogStream.info(
                    ...
                    if np.all(np.asarray(a1bFileCheck_HIS) == True):
                        ...
                        sFileCache_HIS = os.path.join(sPathCache_HIS,
                                                      'Satellite_HSAF-SWI-H16-RAW-STATS_FILENC_' + sTime_SAVE + '.history')
            ...
            if self.bData_SOURCE is True:
                oLogStream.info(
                    ' ====> SAVE DATA AT TIME: ' + sTime_SAVE + ' ...  SKIPPED. Data previously processed!')
            else:
