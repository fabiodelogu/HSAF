...
Name:          Cpl_Apps_Model_DynamicData_HSAF_SoilMoisture_H14_Star_Stats
...
import src.Lib_Var_Apps_Conversion as Lib_Cnv
...
# -------------------------------------------------------------------------------------
# Class
class Cpl_Apps_Model_DynamicData_HSAF_SoilMoisture_H14_Star_Stats:
    ...
    def checkDynamicData(self, sTime):
        ...
        for sFileType in oVarsWorkspace_CHK:
            ...
            if sFileType == 'NetCDF':
                ...
                # Check file
                sFileCache_CHK = os.path.join(sPathCache_CHK, 'Satellite_HSAF-SM-H14-STAR-STATS_FILENC_' + sTime_CHK + '.history')
    ...
    def getDynamicData(self, sTime):
        ...
        #oData_GET = open('oDataDyn_GET_H14-STAR-STATS.pkl', 'wb')
        #pickle.dump(a1oFileDataDyn_GET, oData_GET)
        #oData_GET.close()
        #oGeo_GET = open('oTimeDyn_GET_H14-STAR-STATS.pkl', 'wb')
        #pickle.dump(a1oFileDataGeo_GET, oGeo_GET)
        #oGeo_GET.close()
        #import scipy.io as sio
        #os.chdir(self.sPathClass)
        #oData = {'SM': a1oFileDataDyn_GET['201201010000']}
        #sio.savemat('oDataDyn_GET_H14-STAR-STATS.mat', oData)
        # DEBUG END #####
    ...
    def parserDynamicData(self, sTime):
        ...
        #import pickle
        #os.chdir(self.sPathClass)
        #oData_GET = open('oDataDyn_GET_H14-STAR-STATS.pkl', 'rb')
        #a1oFileDataDyn_GET = pickle.load(oData_GET)
        #oData_GET.close()
        #oGeo_GET = open('oTimeDyn_GET_H14-STAR-STATS.pkl', 'rb')
            ...
            for sVarInfoDyn_PAR in a1oVarsInfoDyn_SOURCE:
                ...
                for iFileTimeDyn_PAR, sFileTimeDyn_PAR in enumerate(sorted(a1oFileDataDyn_GET)):
                    ...
                    if oFileDataDyn_PAR and oFileDataGeo_PAR:
                        ...
                        oVarMethod_Cnv = getattr(Lib_Cnv, sVarMethodCnv_PAR)
                        a2dVar_CNV = oVarMethod_Cnv(a2dVar_INTERP, 0, 100)

                        # Debug
                        #plt.figure(1)
                        #plt.imshow(oVar_PAR); plt.colorbar(); plt.clim(0, 1)
                        #plt.figure(2)
                        #plt.imshow(a2dVar_INTERP); plt.colorbar(); plt.clim(0, 1)
                        #plt.figure(3)
                        #plt.imshow(a2dVar_CNV); plt.colorbar(); plt.clim(0, 100)
                        #plt.show()

                        # Store data and time in workspace(s)
                        a2dVarDataDyn_PAR[:, iFileTimeDyn_PAR] = a2dVar_CNV.ravel()
                ...
                if sFileType == 'NetCDF':
                    ...
                    if np.all(np.asarray(a1bFileCheck_HIS) == True):
                        ...
                        sFileCache_HIS = os.path.join(sPathCache_HIS,
                                                      'Satellite_HSAF-SM-H14-STAR-STATS_FILENC_' + sTime_SAVE + '.history')
